...
class in_mem_MSA_Dataset(Dataset):
    ...
    '''Loads dataset into memory, currently not enough memory to fit but could be useful for later on
       Choose to load only dist, angle or both
       Path should lead to a directory with subdirectories 'Features', 'Dist_Labels', 'Angle_Labels'
    '''
...
def dist_custom_collate(batch, dist_bins=None, crop_size=64, rang=12):
    
    '''Data Loader for only distance information'''
    
    
    feat = torch.zeros(len(batch), crop_size, 100, 22)
    feat_mask = torch.ones(len(batch), crop_size, 100)
    ...
    for i in range(len(batch)):
        ...
        sampled_msa = torch.randperm(msa_len)[:100]
    ...
    return feat,  feat_mask, dist_labels


def angle_dist_custom_collate(batch, dist_bins=None, angle_bins=None, crop_size=64):
    
    
    '''Dataloader for both distances and angles'''
